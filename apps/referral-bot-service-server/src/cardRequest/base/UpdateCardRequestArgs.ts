/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { CardRequestWhereUniqueInput } from "./CardRequestWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { CardRequestUpdateInput } from "./CardRequestUpdateInput";

@ArgsType()
class UpdateCardRequestArgs {
  @ApiProperty({
    required: true,
    type: () => CardRequestWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => CardRequestWhereUniqueInput)
  @Field(() => CardRequestWhereUniqueInput, { nullable: false })
  where!: CardRequestWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => CardRequestUpdateInput,
  })
  @ValidateNested()
  @Type(() => CardRequestUpdateInput)
  @Field(() => CardRequestUpdateInput, { nullable: false })
  data!: CardRequestUpdateInput;
}

export { UpdateCardRequestArgs as UpdateCardRequestArgs };
